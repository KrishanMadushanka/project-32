...
$qty = $qty1;
  ...
  else
  {
    ...
    for($i=0; $i < $N; $i++)
    {
      ...
      $sql="INSERT INTO `smart_shopper`.`deal`(`ItemID`,`I_Name`,`price1`,`price2`,`price3`,`price4`,`price5`,`quantity`) VALUES ('{$item[$i]}',(select I_Name from item where ItemID='".$item[$i]."'),(select price from item where ItemID='".$item[$i]."'),(select price2 from items where ItemID='".$item[$i]."'),(select price3 from items where ItemID='".$item[$i]."'),(select price4 from items where ItemID='".$item[$i]."'),(select price5 from items where ItemID='".$item[$i]."'),'{$qty[$i]}')";
